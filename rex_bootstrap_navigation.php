...
/**
 * Klasse zum Erstellen von Bootstrap-Navigationen, v0.1.
 *
...
class rex_bootstrap_navigation
{
	...
	public function getBreadcrumb($startPageLabel, $includeCurrent = false, $category_id = 0)
	{
		...
		if ($startPageLabel) {
			$lis .= '<li><a href="' . rex_getUrl(rex_article::getSiteStartArticleId()) . '">' . htmlspecialchars($startPageLabel) . '</a></li>';
			++$i;
		...
		foreach ($path as $pathItem) {
			...
			$lis .= '<li><a href="' . $cat->getUrl() . '">' . htmlspecialchars($cat->getName()) . '</a></li>';
			++$i;
		}
		...
		if ($includeCurrent) {
			if ($art = rex_article::get($this->current_article_id)) {
				if (!$art->isStartArticle()) {
					$lis .= '<li>' . htmlspecialchars($art->getName()) . '</li>';
				}
			} else {
				$cat = rex_category::get($this->current_article_id);
				$lis .= '<li>' . htmlspecialchars($cat->getName()) . '</li>';
			}
		}

		return '<ol class="breadcrumb">' . $lis . '</ol>';
	}
	...
	protected function _getNavigation($category_id, $depth = 1)
	{
		...
		foreach ($nav_obj as $nav) {
			...
			if ($this->checkFilter($nav, $depth) && $this->checkCallbacks($nav, $depth, $li, $a)) {
				...
				if (($this->open ||
					...
					$inner = $this->_getNavigation($nav->getId(), $depth);
				}else{
					$inner = '';
				}
				--$depth;
		...
		if (count($lis) > 0) {
			if(1 == $depth){
				return '<ul class="nav navbar-nav me-auto">' . implode('', $lis) . '</ul>';
			}elseif(2 == $depth){
